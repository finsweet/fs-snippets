...
document.addEventListener('DOMContentLoaded', () => __awaiter(void 0, void 0, void 0, function* () {
    const embedElements = document.querySelectorAll('.w-embed');
    for (const [index, embedElement] of embedElements.entries()) {
        let src = embedElement.textContent;
        const titleRegex = /\[(.*)\]/;
        if (!src || (!src.startsWith('http') && !titleRegex.test(src)))
            continue;
        ...
        if (titleMatch) {
            ...
            const titleElement = (0, helpers_1.createDivElement)('codeTitle', title);
